...
from pharmascan.config.settings import (
    ACCENT, ACCENT2, BORDER, CARD, DANGER, DARK, MUTED, TEXT, WARN, PLT_RCPARAMS,
)
PURPLE = "#a78bfa"
BG = DARK
...
from pharmascan.config.settings import COLUMN_MAP, _COLUMN_PATTERNS
...
from pharmascan.components.charts import (
    hbar_chart, time_series_chart, rapid_histogram, build_network_data,
    render_vis_network,
)


from pharmascan.utils.normalization import (
    fmt_number, _toks, _seq_ratio, _tok_fuzzy_subset, _match_score,
    detect_name_clusters, apply_name_normalisation,
    _norm_rama, _rama_similarity, detect_fuzzy_repeat_patients,
)
...
# ── Column normalisation profiler (imported from package) ───────────────────
from pharmascan.processors.data_prep import (
    profile_column, score_column_vs_field, auto_map_columns, apply_column_mapping,
    get_system_fields as _get_system_fields,
)
_SYSTEM_FIELDS = _get_system_fields()

# ── Rules engine (imported from package) ────────────────────────────────────
from pharmascan.processors.rules_engine import run_rules_engine as _rules_engine_pkg

from pharmascan.processors.counter_verification import generate_counter_verification_xlsx
